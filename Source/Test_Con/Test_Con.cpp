...
BOOL MillerRabin(ULONG64 n, ULONG64 k)
{
    if(n == k) return TRUE;
    ULONG64 s, d, b, e, x;
...
void encodeblock(PBYTE Input, PBYTE Output, int len )
{
    static BYTE cb64[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

    Output[0] = cb64[ (int)(Input[0] >> 2) ];
    Output[1] = cb64[ (int)(((Input[0] & 0x03) << 4) | ((Input[1] & 0xf0) >> 4)) ];
    Output[2] = (len > 1 ? cb64[ (int)(((Input[1] & 0x0f) << 2) | ((Input[2] & 0xc0) >> 6)) ] : '=');
    Output[3] = (len > 2 ? cb64[ (int)(Input[2] & 0x3f) ] : '=');
}

ULONG_PTR
Base64Encode(
    PVOID       InputBuffer,
    ULONG_PTR   InputLength,
    PVOID       OutputBuffer,
    ULONG_PTR   OutputLength,
    ULONG_PTR   LineSize = ~0u
)
{
    BYTE    LocalBuffer[4];
    PBYTE   Input, Output;
    ULONG_PTR len, blocksout;

	*LocalBuffer = 0;
    blocksout = 0;
    Input = (PBYTE)InputBuffer;
    Output = (PBYTE)OutputBuffer;

    for (ULONG_PTR Length = InputLength; Length;)
    {
        len = ML_MIN(Length, 3);

        encodeblock(Input, Output, len);
        blocksout++;
        Output += 4;
        Input += len;
        Length -= len;
    ...
    return PtrOffset(Output, OutputBuffer);
}
...
ForceInline Void main2(LongPtr argc, TChar **argv)
{
    ...
    LOOP_FOREVER
    {
        sn = GetRandom32Range(100000000, 999999999);
        if (IsPrime(sn))
            break;
    }
    ...
    argc = Base64Encode(&sn, sizeof(sn), base64, sizeof(base64));
    argc = Base64Encode(base64, argc, base46, sizeof(base46));
