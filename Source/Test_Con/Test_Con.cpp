...
VOID OnDeviceConnected(HANDLE Device)
{
    NTSTATUS            Status;
    PVOID               Grappa;
    ...
    Grappa = nullptr;
    Length = 0;
    Status = GetGrappaInfo(&init, &GrappaKey, &Grappa, &Length);
    if (Status != STATUS_SUCCESS)
        return;

    // RequestingSync['Params']['HostInfo']['Grappa'] = Base64Encode(Grappa, Length);

    AMDServiceConnectionSend(Connection, RequestingSync, sizeof(RequestingSync));

    Length = AMDServiceConnectionReceive(Connection, &Length, sizeof(Length));

    PBYTE Buffer = (PBYTE)AllocStack(Length);

    AMDServiceConnectionReceive(Connection, Buffer, Length);
}
...
VOID Direct()
{
    ...
    *(PVOID *)&EstablishGrappaKey   = PtrAdd(AirTrafficHost, 0x7D90);
    ...
    AMDeviceNotificationSubscribe(
        [] (PDEVICE_CONNECTION_INFO Info) -> VOID
        {
            switch (Info->State)
            {
