...
NTSTATUS QueryFileNameFromSectionHandle(HANDLE Section, PUNICODE_STRING FileName)
{
    NTSTATUS                            Status;
    PVOID                               BaseAddress;
    ULONG_PTR                           ViewSize;
    SECTION_IMAGE_INFORMATION           ImageInfo;
    PMEMORY_MAPPED_FILENAME_INFORMATION ImageName;

    Status = NtQuerySection(Section, SectionImageInformation, &ImageInfo, sizeof(ImageInfo), nullptr);
    FAIL_RETURN(Status);

    BaseAddress = nullptr;
    ViewSize = 0;
    NtMapViewOfSection(Section, CurrentProcess, &BaseAddress, 0, MEMORY_PAGE_SIZE, nullptr, &ViewSize, ViewShare, 0, PAGE_EXECUTE);
    FAIL_RETURN(Status);

    ViewSize = 0x10;
    ImageName = (PMEMORY_MAPPED_FILENAME_INFORMATION)AllocateMemory(ViewSize);

    LOOP_ONCE
    {
        Status = NtQueryVirtualMemory(CurrentProcess, BaseAddress, MemoryMappedFilenameInformation, ImageName, ViewSize, &ViewSize);
        ...
        ImageName = (PMEMORY_MAPPED_FILENAME_INFORMATION)ReAllocateMemory(ImageName, ViewSize);
        ...
        Status = NtQueryVirtualMemory(CurrentProcess, BaseAddress, MemoryMappedFilenameInformation, ImageName, ViewSize, nullptr);
    }
    ...
    if (NT_SUCCESS(Status))
    {
        RtlDuplicateUnicodeString(RTL_DUPSTR_ADD_NULL, &ImageName->Name, FileName);
    }

    FreeMemoryP(ImageName);

    return Status;
}

PWSTR LookupProcess(PWSTR FullPath)
{
    return L"print_args.exe";
}

NTSTATUS
QueryImagePathAndCommandLine(
    IN  PUNICODE_STRING ImagePathName,
    IN  PUNICODE_STRING CommandLine,
    OUT PUNICODE_STRING ImagePath,
    OUT PUNICODE_STRING CmdLine
)
{
    RtlCreateUnicodeString(ImagePath, L"C:\\print_args.exe");

    if (CommandLine != nullptr && CmdLine != nullptr)
        RtlDuplicateUnicodeString(RTL_DUPSTR_ADD_NULL, CommandLine, CmdLine);
...
NTSTATUS
NTAPI
RtlCreateProcessParametersEx_Nt6(
...
)
{
    ...
    RtlInitEmptyString(&ImagePath);
    RtlInitEmptyString(&CmdLine);

    Status = QueryImagePathAndCommandLine(ImagePathName, CommandLine, &ImagePath, &CmdLine);
    if (NT_SUCCESS(Status))
    {
    ...
    Status = StubRtlCreateProcessParametersEx(ProcessParameters, ImagePathName, DllPath, CurrentDirectory, CommandLine, Environment, WindowTitle, DesktopInfo, ShellInfo, RuntimeData, Flags);
...
{
    NTSTATUS        Status;
    UNICODE_STRING  ImageDosPath, ImagePath, ImageName;
    PPS_ATTRIBUTE   Attribute, AttributeImageName;

    RtlInitEmptyString(&ImagePath);

    AttributeImageName = nullptr;
    ...
    LOOP_ONCE
    {
        ...
        for (ULONG_PTR Count = (PPS_ATTRIBUTE)PtrAdd(AttributeList, AttributeList->TotalLength) - Attribute;
             Count != 0;
             ++Attribute, --Count)
        {
            if (Attribute->AttributeNumber == PsAttributeImageName)
            {
                AttributeImageName = Attribute;
                break;
            }
        }

        if (AttributeImageName == nullptr)
            break;

        ImageName.Buffer        = (PWSTR)AttributeImageName->ValuePtr;
        ImageName.Length        = AttributeImageName->Size;
        ImageName.MaximumLength = AttributeImageName->Size;

        Status = Rtl::NtPathNameToDosPathName(&ImageDosPath, &ImageName);
        FAIL_BREAK(Status);

        Status = QueryImagePathAndCommandLine(&ImageDosPath, nullptr, &ImagePath, nullptr);
        RtlFreeUnicodeString(&ImageDosPath);
        FAIL_BREAK(Status);

        ImageDosPath = ImagePath;
        Status = NtFileDisk::QueryFullNtPath(ImagePath.Buffer, &ImagePath);
        RtlFreeUnicodeString(&ImageDosPath);
        FAIL_BREAK(Status);

        AttributeImageName->ValuePtr = ImagePath.Buffer;
        AttributeImageName->Size = ImagePath.Length;
    }

    Status = StubNtCreateUserProcess(ProcessHandle, ThreadHandle, ProcessDesiredAccess, ThreadDesiredAccess, ProcessObjectAttributes, ThreadObjectAttributes, ProcessFlags, ThreadFlags, ProcessParameters, CreateInfo, AttributeList);

    RtlFreeUnicodeString(&ImagePath);

    if (AttributeImageName != nullptr)
    {
        AttributeImageName->ValuePtr = ImageName.Buffer;
        AttributeImageName->Size = ImageName.Length;
    }
...
)
{
    ...
    Status = QueryFileNameFromSectionHandle(SectionHandle, &ProcessFullPath);

    if (NT_SUCCESS(Status))
    {
        AllocConsole();
        PrintConsole(L"%wZ\n", &ProcessFullPath);
        RtlFreeUnicodeString(&ProcessFullPath);
    }

    return StubNtCreateProcessEx(ProcessHandle, DesiredAccess, ObjectAttributes, ParentProcess, Flags, SectionHandle, DebugPort, ExceptionPort, JobMemberLevel);
}

NTSTATUS
NTAPI
RtlCreateProcessParameters_Nt5(
    PRTL_USER_PROCESS_PARAMETERS*   ProcessParameters,
    PUNICODE_STRING                 ImagePathName,
    PUNICODE_STRING                 DllPath,
    PUNICODE_STRING                 CurrentDirectory,
...
)
{
    ...
    Status = StubRtlCreateProcessParameters(ProcessParameters, ImagePathName, DllPath, CurrentDirectory, CommandLine, Environment, WindowTitle, DesktopInfo, ShellInfo, RuntimeData);
...
PVOID HookModuleIAT(PVOID Base, ULONG RoutineHash, PVOID NewRoutine)
{
    PVOID       IAT, OldRoutine;
    ULONG       Protect;
    NTSTATUS    Status;

    IAT = (PVOID)IATLookupRoutineRVAByHashNoFix(Base, RoutineHash);
    ...
    OldRoutine = _InterlockedExchangePointer(IAT, NewRoutine);
...
NTSTATUS HookCreateProcessNT5OrNt6(PVOID Kernel32)
{
    *(PVOID *)&StubRtlCreateProcessParametersEx = HookModuleIAT(Kernel32, NTDLL_RtlCreateProcessParametersEx, RtlCreateProcessParametersEx_Nt6);
    if (StubRtlCreateProcessParametersEx != nullptr)
    {
        *(PVOID *)&StubNtCreateUserProcess = HookModuleIAT(Kernel32, NTDLL_NtCreateUserProcess, NtCreateUserProcess_Nt6);

        return StubNtCreateUserProcess != nullptr ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
    }

    *(PVOID *)&StubNtCreateProcessEx = HookModuleIAT(Kernel32, NTDLL_NtCreateProcessEx, NtCreateProcessEx_Nt5);
    if (StubNtCreateProcessEx == nullptr)
        return STATUS_UNSUCCESSFUL;

    *(PVOID *)&StubRtlCreateProcessParameters = HookModuleIAT(Kernel32, NTDLL_RtlCreateProcessParameters, RtlCreateProcessParameters_Nt5);

    return StubRtlCreateProcessParameters != nullptr ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
}

NTSTATUS HookCreateProcessNT62(PVOID KernelBase)
{
    *(PVOID *)&StubNtCreateUserProcess = HookModuleIAT(KernelBase, NTDLL_NtCreateUserProcess, NtCreateUserProcess_Nt6);
    if (StubNtCreateUserProcess == nullptr)
        return STATUS_UNSUCCESSFUL;

    *(PVOID *)&StubRtlCreateProcessParametersEx = HookModuleIAT(KernelBase, NTDLL_RtlCreateProcessParametersEx, RtlCreateProcessParametersEx_Nt6);

    return StubRtlCreateProcessParametersEx != nullptr ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
}

NTSTATUS HookCreateProcess()
{
    PVOID KernelBase;
    NTSTATUS Status;
    ...
    {
        Status = HookCreateProcessNT62(KernelBase);
        SUCCESS_RETURN(Status);
    }

    return HookCreateProcessNT5OrNt6(GetModuleHandleW(L"KERNEL32.dll"));
}
...
ForceInline Void main2(LongPtr argc, TChar **argv)
{
    ...
    Ps::CreateProcess(nullptr, L"notepad2.exe");
