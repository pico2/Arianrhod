...
PVOID NTAPI ModiLoadLibraryW(PCWSTR DllName)
{
    UNICODE_STRING Dll;
    static UNICODE_STRING Mso = RTL_CONSTANT_STRING(L"MSO.dll");

    RtlInitUnicodeString(&Dll, DllName);

    LOOP_ONCE
    {
        if (Dll.Length < Mso.Length)
            break;

        *(PULONG_PTR)&Dll.Buffer += Dll.Length - Mso.Length;
        Dll.Length = Mso.Length;

        if (RtlEqualUnicodeString(&Dll, &Mso, TRUE) != FALSE)
            DllName = Dll.Buffer;
    }

    return Ldr::LoadDll(DllName);
}
...
NTSTATUS OcrTiff(PCWSTR TiffFile, ml::String &ResultText)
{
    ...
    LOOP_ONCE
    {
        ...
        PrintConsole(L"get_Images\n");

        hr = doc->get_Images(&imgs);
        FAIL_BREAK(hr);

        PrintConsole(L"imgs->get_Item\n");

        hr = imgs->get_Item(0, (IDispatch**)&img);
        FAIL_BREAK(hr);

        PrintConsole(L"img->get_Layout\n");

        hr = img->get_Layout(&layout);
        FAIL_BREAK(hr);

        PrintConsole(L"layout->get_Words\n");

        hr = layout->get_Words(&words);
        FAIL_BREAK(hr);

        PrintConsole(L"words->get_Count\n");
...
ForceInline Void main2(LongPtr argc, TChar **argv)
{
    ...
    if (NT_FAILED(Status))
    {
        PrintConsole(L"init failed: %p\n", Status);
        PauseConsole();
        return;
    }

    MODI::OcrTiff(LR"(Î´±êÌâ-1.tif)", ret);
