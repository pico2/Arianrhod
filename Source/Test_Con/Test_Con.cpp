...
typedef struct
{
    UNICODE_STRING  GameProcessName;
    UNICODE_STRING  GameName;
    ULONG_PTR       NumberOfSubFile;
    PUNICODE_STRING SubFileList;

    // reg info

    UNICODE_STRING  GameExeRelativePath;
    HKEY            RootKey;
    UNICODE_STRING  SubKey;
    UNICODE_STRING  ValueName;

} STATIC_GAME_INFO, *PSTATIC_GAME_INFO;

UNICODE_STRING LolFileList[] =
{
    RTL_CONSTANT_STRING(L"..\\Game\\League of Legends.exe"),
};

STATIC_GAME_INFO LolLauncherInfo =
{
    RTL_CONSTANT_STRING(L"Client.exe"),
    RTL_CONSTANT_STRING(L"Ó¢ÐÛÁªÃË"),
    countof(LolFileList),
    LolFileList,

    RTL_CONSTANT_STRING(L""),
    HKEY_LOCAL_MACHINE,
    RTL_CONSTANT_STRING(L"SOFTWARE\\Tencent\\LOL"),
    RTL_CONSTANT_STRING(L"setup"),
};

NTSTATUS CheckSubFileList(PWSTR GameExeFullPath, PSTATIC_GAME_INFO GameInfo)
{
    if (PathFileExistsW(GameExeFullPath) == FALSE ||
        wcsicmp(PathFindFileNameW(GameExeFullPath), GameInfo->GameProcessName.Buffer) != 0)
    {
        return STATUS_NOT_FOUND;
    }

    PWSTR           Name;
    PUNICODE_STRING FileList;
    ml::String      path;

    {
        WCHAR ch;

        Name = PathFindFileNameW(GameExeFullPath);
        ch = *Name;
        *Name = 0;
        path = GameExeFullPath;
        *Name = ch;
    }

    FileList = GameInfo->SubFileList;
    for (ULONG_PTR Count = GameInfo->NumberOfSubFile; Count != 0; ++FileList, --Count)
    {
        if (PathFileExistsW(path + FileList->Buffer) == FALSE)
            return STATUS_NOT_FOUND;
    }
...
NTSTATUS SearchFromRegistry(PUNICODE_STRING GameFullPath, PSTATIC_GAME_INFO GameInfo)
{
    HKEY            Key;
    LONG            Status;
    ULONG           Size;
    ...
    Status = RegOpenKeyExW(GameInfo->RootKey, GameInfo->SubKey.Buffer, 0, KEY_QUERY_VALUE, &Key);
    if (Status != NO_ERROR)
        return STATUS_UNSUCCESSFUL;

    BufferSize = MAX_NTPATH * sizeof(Buffer[0]);
    Buffer = (PWSTR)AllocateMemory(BufferSize);
    if (Buffer == NULL)
    {
        RegCloseKey(Key);
        return STATUS_NO_MEMORY;
    ...
    {
        Size = BufferSize;
        Status = RegQueryValueExW(Key, GameInfo->ValueName.Buffer, NULL, NULL, (PBYTE)Buffer, &Size);
        if (Status != ERROR_MORE_DATA)
            break;

        BufferSize = Size + GameInfo->GameExeRelativePath.MaximumLength;
        Buffer = (PWSTR)ReAllocateMemory(Buffer, BufferSize);
        if (Buffer == NULL)
            break;
    }
    ...
    CopyMemory(PtrAdd(Buffer, Size), GameInfo->GameExeRelativePath.Buffer, GameInfo->GameExeRelativePath.Length);
    *PtrAdd(Buffer, Size + GameInfo->GameExeRelativePath.Length) = 0;

    Status = CheckSubFileList(Buffer, GameInfo);

    if (NT_SUCCESS(Status))
    {
        Status = RtlCreateUnicodeString(GameFullPath, Buffer) ? STATUS_SUCCESS : STATUS_NO_MEMORY;
    }
...
LONG NTAPI EnumFilesCallback(PVOID, PWIN32_FIND_DATAW FindData, GrowableArray<ml::String> &FileList)
{
    if (wcsicmp(PathFindExtensionW(FindData->cFileName), L".lnk") != 0)
        return 0;
...
VOID
SearchPathLinks(
    GrowableArray<ml::String>&  FileList,
    ULONG_PTR                   CurrentUserPathId,
    ULONG_PTR                   CommonPathId,
    ULONG_PTR                   SearchFlags
)
{
    HRESULT result;
    WCHAR CurrentUser[MAX_NTPATH], Common[MAX_NTPATH];

    result = SHGetFolderPathW(NULL, CurrentUserPathId, NULL, 0, CurrentUser);
    if (SUCCEEDED(result))
    {
        EnumDirectoryFiles(NULL, L"*.*", 0, CurrentUser, NULL, (FEnumDirectoryFilesCallBack)EnumFilesCallback, (ULONG_PTR)&FileList, SearchFlags);
    }

    result = SHGetFolderPathW(NULL, CommonPathId, NULL, 0, Common);
    if (SUCCEEDED(result) && wcsicmp(CurrentUser, Common) != 0)
    {
        EnumDirectoryFiles(NULL, L"*.*", 0, Common, NULL, (FEnumDirectoryFilesCallBack)EnumFilesCallback, (ULONG_PTR)&FileList, SearchFlags);
    }
}

NTSTATUS SearchFromDesktopAndStartMenu(PUNICODE_STRING GameFullPath, PSTATIC_GAME_INFO GameInfo)
{
    WCHAR FullPath[MAX_NTPATH];
    GrowableArray<ml::String> FileList;
    ml::String *File;

    SearchPathLinks(FileList, CSIDL_DESKTOPDIRECTORY, CSIDL_COMMON_DESKTOPDIRECTORY, 0);
    SearchPathLinks(FileList, CSIDL_STARTMENU, CSIDL_COMMON_STARTMENU, EDF_SUBDIR);

    if (FileList.GetSize() == 0)
        return STATUS_NOT_FOUND;

    File = FileList.GetData();
    for (ULONG_PTR Count = FileList.GetSize(); Count != 0; ++File, --Count)
    {
        if (NT_FAILED(GetPathFromLinkFile(*File, FullPath, countof(FullPath))))
            continue;

        if (NT_FAILED(CheckSubFileList(FullPath, GameInfo)))
            continue;

        return RtlCreateUnicodeString(GameFullPath, FullPath) ? STATUS_SUCCESS : STATUS_NO_MEMORY;
    }

    return STATUS_NOT_FOUND;
}

NTSTATUS SearchGame(PUNICODE_STRING GameFullPath, PSTATIC_GAME_INFO GameInfo)
{
    SUCCESS_RETURN(SearchFromRegistry(GameFullPath, GameInfo));
    SUCCESS_RETURN(SearchFromDesktopAndStartMenu(GameFullPath, GameInfo));
...
ForceInline Void main2(LongPtr argc, TChar **argv)
{
    ...
    RtlInitEmptyString(&lol);

    Status = SearchGame(&lol, &LolLauncherInfo);
    PrintConsole(L"%wZ\n", &lol);
