...
def main():
    ...
    QueueWorkItem(0x101, 1, lambda_346)
    ...
    def Function_3_426(): pass

    label("Function_3_426")
    ...
    QueueWorkItem2(0x101, 1, lambda_438)
    ...
    QueueWorkItem2(0x102, 1, lambda_449)

    def lambda_45A():
        OP_8E(0x101, 0x1234, 0x0, 0x3494, 0x1388, 0x0)
        ExitThread()

    QueueWorkItem(0x101, 2, lambda_45A)

    def lambda_475():
        OP_8E(0x102, 0x1266, 0x0, 0x30C0, 0x1388, 0x0)
        ExitThread()

    QueueWorkItem(0x102, 2, lambda_475)
    ...
    QueueWorkItem(0x102, 3, lambda_554)
    ...
    QueueWorkItem(0x101, 3, lambda_5D6)
    ...
    QueueWorkItem(0xA, 1, lambda_672)
    Sleep(400)
    ...
    QueueWorkItem(0x102, 1, lambda_693)

    def lambda_6A1():
        OP_8C(0xFE, 0, 400)
        ExitThread()

    QueueWorkItem(0x9, 1, lambda_6A1)

    def lambda_6AF():
        OP_8C(0xFE, 0, 400)
        ExitThread()

    QueueWorkItem(0x8, 1, lambda_6AF)

    def lambda_6BD():
        OP_6D(890, 0, 18340, 6000)
        ExitThread()

    QueueWorkItem(0x101, 1, lambda_6BD)

    def lambda_6D5():
        OP_67(0, 3790, -10000, 6000)
        ExitThread()

    QueueWorkItem(0x101, 2, lambda_6D5)

    def lambda_6ED():
        OP_6C(60000, 6000)
        ExitThread()

    QueueWorkItem(0x101, 3, lambda_6ED)
    OP_22(0x92, 0x0, 0x64)
    ...
    QueueWorkItem(0xB, 2, lambda_799)

    def lambda_7A7():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0x258, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_7A7)
    Sleep(200)
    ...
    QueueWorkItem(0xB, 2, lambda_7C7)

    def lambda_7D5():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0x3E8, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_7D5)
    Sleep(500)
    ...
    QueueWorkItem(0xB, 2, lambda_7F5)

    def lambda_803():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0x5DC, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_803)

    def lambda_81E():
        OP_8C(0xFE, 135, 13)
        ExitThread()

    QueueWorkItem(0xB, 2, lambda_81E)
    Sleep(100)
    ...
    QueueWorkItem(0xB, 1, lambda_831)
    Sleep(100)
    ...
    QueueWorkItem(0xB, 2, lambda_851)

    def lambda_85F():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0x7D0, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_85F)
    Sleep(250)
    ...
    QueueWorkItem(0xB, 2, lambda_87F)

    def lambda_88D():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0x7D0, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_88D)
    Sleep(250)
    ...
    QueueWorkItem(0xB, 2, lambda_8AD)

    def lambda_8BB():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0xBB8, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_8BB)
    Sleep(250)
    ...
    QueueWorkItem(0xB, 2, lambda_8DB)

    def lambda_8E9():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0xBB8, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_8E9)
    Sleep(300)
    ...
    QueueWorkItem(0xB, 2, lambda_909)
    ...
    QueueWorkItem(0xB, 2, lambda_937)

    def lambda_945():
        OP_91(0xFE, 0xFFFFF448, 0x0, 0xBB8, 0x5DC, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_945)
    Sleep(100)
    ...
    QueueWorkItem(0xB, 2, lambda_965)

    def lambda_973():
        OP_91(0xFE, 0x0, 0x0, 0x2710, 0x7D0, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_973)
    Sleep(300)
    ...
    QueueWorkItem(0xB, 2, lambda_9C8)

    def lambda_9D6():
        OP_91(0xFE, 0x0, 0x0, 0x2710, 0xFA0, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_9D6)
    Sleep(300)
    ...
    QueueWorkItem(0xB, 1, lambda_9F6)

    def lambda_A11():
        OP_8C(0xFE, 180, 20)
        ExitThread()

    QueueWorkItem(0xB, 2, lambda_A11)
    Sleep(300)
    ...
    QueueWorkItem(0xB, 2, lambda_A24)

    def lambda_A32():
        OP_91(0xFE, 0x0, 0x0, 0x2710, 0x2328, 0x0)
        ExitThread()

    QueueWorkItem(0xB, 1, lambda_A32)
    Sleep(1000)
    OP_20(0x7D0)
    FadeToDark(1500, 0, -1)
    OP_0D()
