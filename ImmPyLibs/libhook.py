...
class Hook:
    ...
    def unhook2(self, desc = None):
        if not desc:
            desc = self.desc
        debugger.RemoveHook(self, desc)

    def unhookall(self):
        return self.UnHook2(self.desc)
...
class BpHook(Hook):
    ...
    def unhook2(self, desc = None):
        if not desc:
            if not self._this_eip:
                return

            desc = self.descdict[self._this_eip]
            if not desc:
                return
            del self.descdict[self._this_eip]

        debugger.RemoveHook(self, desc)

    def unhookall(self):
        for addr, desc in self.descdict.iteritems():
            self.UnHook2(desc)
        self.descdict.clear()
    ...
    def check_run(self, regs):
        if self.enabled:
            self._this_eip = regs['EIP']
            self._run(regs)
            self._this_eip = None
        
class LogBpHook(BpHook):
    def __init__(self):
